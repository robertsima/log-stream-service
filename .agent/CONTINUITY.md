...
- 2026-07-07 [CODE] Dashboard alert-analysis panel now accepts raw log lines (frontend-only): pasting console output (e.g. Keycloak/Quarkus logs) previously failed with "Events JSON is invalid" because the input was JSON.parse-only. `parseAnalysisEvents` in dashboard.js now falls back to a plaintext line parser (`parsePlainTextLogLines`) when input doesn't start with `[`/`{`: detects leading timestamps (space or T, comma or dot millis → ISO), level tokens (SEVERE now maps to ERROR too), `[logger]`, and folds stack-trace continuation lines (indented / `at ` / `Caused by:` / exception-class lines) into the previous event. Single JSON object also wrapped into array. dashboard.html copy updated. Verified with node harness against the exact failing paste + stack-trace sample; `node --check` clean. Note: analyze still requires ≥1 ERROR event by design (production buckets only aggregate errors).
