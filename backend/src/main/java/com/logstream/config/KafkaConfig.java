...
public class KafkaConfig {
    ...
    // log stream: central-log-events -> normalize -> alert context -> alert-messages
    @Bean
    public KStream<String, LogEvent> centralLogStream(
        ...
        // Producer already keys by appId — no selectKey/repartition (avoids an internal topic).
        // flatMapValues drops unusable events without killing the Streams thread.
        logs.flatMapValues(normalizer::normalizeOrDrop)
                .peek((appId, event) -> log.debug(
                        "[streams] normalized appId={} level={}",
                        appId,
                        event.payload() != null ? event.payload().path("level").asText("") : ""))
                .process(new AlertContextProcessor.Supplier())
                .peek((appId, alert) -> log.info(
                        "[streams] alert-trigger appId={} occurrences={} label={}",
                        appId,
