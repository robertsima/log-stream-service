...
3. Pick agent(s) from `.agent/manifests/` using **inputs/outputs**, not domain guesses.
4. Enforce: no implementer starts until `.agent/contracts/<slug>.md` exists and lists affected surfaces.
5. After implementation → route to **reviewer** when a contract was used.
...
Always loaded: `000-project-context`, `010-workflow-and-continuity`.

## Verification

| Area | Command |
|------|---------|
| Backend build | `cd backend && mvn -q clean package -DskipTests` |
| Backend tests | `cd backend && mvn -q test` |
| Frontend | No build step; manual check in browser |
